...
func InsertToTbl(strDbName string, strTblName string, poDb *sql.DB, poIn *CCG_LoginLogDb) bool{ 
	//check
	if len(strDbName) <= 0 {
...
//传入参数：表名，db操作指针，数据结构
func FInsToTbl(strTblName string, poDb *sql.DB, poIn *CCG_LoginLogDb) bool{ 
	strSql := "insert INTO " + strTblName
	strSql += "( SortId, UsrId, Ip, DateTime, DeviceType )"
	strSql += " values( '" + strconv.Itoa( poIn.SortId ) + "', '" + strconv.Itoa( poIn.UsrId ) + "', '" + poIn.Ip + "', '" + poIn.DateTime + "', '" + strconv.Itoa( poIn.DeviceType ) + "')"
...
//查询类接口，都是快速查询，因为查询用的比较多
//快速获取所有
func FGetAll(strTblName string, poDb *sql.DB) []CCG_LoginLogDb { 
	_rr := make([]CCG_LoginLogDb, 0)
...
//快速获取 通过SortId获取某一行
func FGetBySortId( strTblName string, poDb *sql.DB, iSortId int ) ( CCG_LoginLogDb, bool ) { 
	_oIn := CCG_LoginLogDb{}
...
//快速获取 通过UsrId获取某一行
func FGetByUsrId( strTblName string, poDb *sql.DB, iUsrId int ) ( CCG_LoginLogDb, bool ) { 
	_oIn := CCG_LoginLogDb{}
...
//快速获取 通过Ip获取某一行
func FGetByIp( strTblName string, poDb *sql.DB, strIp string ) ( CCG_LoginLogDb, bool ) { 
	_oIn := CCG_LoginLogDb{}
...
func FGetByDateTime( strTblName string, poDb *sql.DB, strDateTime string ) ( CCG_LoginLogDb, bool ) { 
	_oIn := CCG_LoginLogDb{}
...
func FGetByDeviceType( strTblName string, poDb *sql.DB, iDeviceType int ) ( CCG_LoginLogDb, bool ) { 
	_oIn := CCG_LoginLogDb{}
...
//更新，只有快速接口，更新某一行的所有，以及更新某一列
//快速更新,通过主key，整个行，不检查
func FUpRowByPriKey( strTblName string, poDb *sql.DB, poUp *CCG_LoginLogDb, iSortId int ) bool { 
...
func FUpRowByUsrId( strTblName string, poDb *sql.DB, poUp *CCG_LoginLogDb, iUsrId int ) bool { 
	strSql := "update " + strTblName + " set "
...
//快速更新,通过Ip，整个行，不检查
func FUpRowByIp( strTblName string, poDb *sql.DB, poUp *CCG_LoginLogDb, strIp string ) bool { 
	strSql := "update " + strTblName + " set "
	strSql += "SortId= '" + strconv.Itoa( poUp.SortId ) + "', " 
	strSql += "UsrId= '" + strconv.Itoa( poUp.UsrId ) + "', " 
	strSql += "DateTime= '" + poUp.DateTime + "', " 
...
func FUpRowByDateTime( strTblName string, poDb *sql.DB, poUp *CCG_LoginLogDb, strDateTime string ) bool { 
	strSql := "update " + strTblName + " set "
	strSql += "SortId= '" + strconv.Itoa( poUp.SortId ) + "', " 
	strSql += "UsrId= '" + strconv.Itoa( poUp.UsrId ) + "', " 
	strSql += "Ip= '" + poUp.Ip + "', " 
...
func FUpRowByDeviceType( strTblName string, poDb *sql.DB, poUp *CCG_LoginLogDb, iDeviceType int ) bool { 
	strSql := "update " + strTblName + " set "
	strSql += "SortId= '" + strconv.Itoa( poUp.SortId ) + "', " 
	strSql += "UsrId= '" + strconv.Itoa( poUp.UsrId ) + "', " 
	strSql += "Ip= '" + poUp.Ip + "', " 
...
func FUpUsrIdByPriKey( strTblName string, poDb *sql.DB, poUp *CCG_LoginLogDb, iUsrId int, iSortId int ) bool { 
	strSql := "update " + strTblName + " set "
	strSql += "UsrId='" + strconv.Itoa( poUp.UsrId ) + "'" 
	strSql += "where SortId= '" + strconv.Itoa( iSortId ) + "'" 
	_, _e := poDb.Exec(strSql)
...
//如果要更新多列，还需要增加接口
//快速更新,通过主key，单cell，不检查
func FUpIpByPriKey( strTblName string, poDb *sql.DB, poUp *CCG_LoginLogDb, strIp string, iSortId int ) bool { 
	strSql := "update " + strTblName + " set "
	strSql += "Ip='" + poUp.Ip + "'" 
	strSql += "where SortId= '" + strconv.Itoa( iSortId ) + "'" 
	_, _e := poDb.Exec(strSql)
...
//如果要更新多列，还需要增加接口
//快速更新,通过主key，单cell，不检查
func FUpDateTimeByPriKey( strTblName string, poDb *sql.DB, poUp *CCG_LoginLogDb, strDateTime string, iSortId int ) bool { 
	strSql := "update " + strTblName + " set "
	strSql += "DateTime='" + poUp.DateTime + "'" 
	strSql += "where SortId= '" + strconv.Itoa( iSortId ) + "'" 
	_, _e := poDb.Exec(strSql)
...
//如果要更新多列，还需要增加接口
//快速更新,通过主key，单cell，不检查
func FUpDeviceTypeByPriKey( strTblName string, poDb *sql.DB, poUp *CCG_LoginLogDb, iDeviceType int, iSortId int ) bool { 
	strSql := "update " + strTblName + " set "
	strSql += "DeviceType='" + strconv.Itoa( poUp.DeviceType ) + "'" 
	strSql += "where SortId= '" + strconv.Itoa( iSortId ) + "'" 
	_, _e := poDb.Exec(strSql)
